...
def complaints(request):
    ...
    if request.method == 'POST':
        ...
        if complaint_form.is_valid():
               ...
               #mail
               print(mail)
               send_mail('Hi Ticket has been Raised', 'Thank you for letting us know of your concern, Our team is working on it to its best.  Dont Reply to this mail', 'settings.EMAIL_HOST_USER', [mail],fail_silently=False)
               profile = Profile.objects.get(user=request.user)
               phone_number=profile.contactnumber
               country_code = '+91'
               mobile_number=f'{country_code} {phone_number}'
               #to_whatsapp_number = f'+{country_code}{phone_number}' # Replace with the actual recipient's number
               #message_body = f'New complaint registered: {instance.Subject} - {instance.Description}'

               #Schedule the message to be sent from now
               now = datetime.now()
               scheduled_time = now + timedelta(minutes=2)

                # Extract hours and minutes from the scheduled time
               scheduled_hours, scheduled_minutes = scheduled_time.hour, scheduled_time.minute
               #hours, minutes = now.hour, now.minute
               #kit.sendwhatmsg(to_whatsapp_number, message_body, hours, minutes)
               try:
                # Replace 'XXXXXXXXXX' with the recipient's phone number
                # The time is in 24-hour format
                        kit.sendwhatmsg(mobile_number, "Hi Ticket has been Raised', 'Thank you for letting us know of your concern, Our team is working on it to its best.  Dont Reply to this mail", scheduled_hours, scheduled_minutes)
...
def allcomplaints(request):
        ...
        if request.method=='POST':
                ...
                if forms.is_valid():
                        ...
                        print(m)
                        send_mail('Hi, Complaint has been Resolved ', 'Thanks for letting us know of your concern, Hope we have solved your issue. Dont Reply to this mail', 'settings.EMAIL_HOST_USER', [m],fail_silently=False)
                        obj.save()
